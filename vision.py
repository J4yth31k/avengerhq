    ...
    def format_report(self, analysis: dict) -> str:
        ...
        for pair, data in analysis.get('pairs', {}).items():
            ...
            if ob:
                ...
                if bid_wall:
                    lines.append(
                        f'    Bid Wall: `{esc(f"${bid_wall[\"price\"]:,.2f}")}` '
                        f'\\({esc(f"{bid_wall[\"size\"]:.2f}")} qty\\)'
                    )
                if ask_wall:
                    lines.append(
                        f'    Ask Wall: `{esc(f"${ask_wall[\"price\"]:,.2f}")}` '
                        f'\\({esc(f"{ask_wall[\"size\"]:.2f}")} qty\\)'
                    )
            ...
            if vp:
                ...
                lines.append(f'    POC: `{esc(f"${vp[\"poc\"]:,.2f}")}`')
                lines.append(f'    VAH: `{esc(f"${vp[\"vah\"]:,.2f}")}`')
                lines.append(f'    VAL: `{esc(f"${vp[\"val\"]:,.2f}")}`')
                lines.append(f'    VWAP: `{esc(f"${vp[\"vwap\"]:,.2f}")}`')

            # Open interest
            oi = data.get('open_interest')
            if oi:
                lines.append(f'  *{esc("Open Interest")}*')
                lines.append(f'    OI: `{esc(f"{oi[\"current_oi\"]:,.2f}")}` contracts')
                if oi.get('current_oi_value_usd'):
                    lines.append(f'    OI Value: `{esc(f"${oi[\"current_oi_value_usd\"]:,.0f}")}`')
